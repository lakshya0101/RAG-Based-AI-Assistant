"""
Step 4b of the RAG pipeline: Embed merged JSON chunks via Ollama.

Reads merged segment JSON from `new_jsons/`, calls the embedding model via Ollama,
and saves a pandas DataFrame to `new_embeddings.joblib` for vector retrieval.
"""
...
from config import EMBED_MODEL, NEW_JSONS_DIR, OLLAMA_BASE_URL

BATCH_SIZE = 8
OUTPUT_FILE = Path("new_embeddings.joblib")
...
def main() -> None:
    ...
    json_files = [f for f in NEW_JSONS_DIR.iterdir() if f.is_file() and f.suffix.lower() == ".json"]

    if not json_files:
        print(f"No merged JSON files found in {NEW_JSONS_DIR.resolve()}.")
        print("Run `merge_chunks.py` first, or run `generate_sample_data.py` to create sample course data.")
        return

    records = []
    chunk_id = 0

    print(f"Embedding merged chunks with model '{EMBED_MODEL}' from {NEW_JSONS_DIR.name}...")
